...
import { simpleOAuth } from './server/oauth-simple';
...
async function checkDataIntegrity() {
  ...
  try {
    ...
    if (orphanedNotes.rows[0].count > 0) {
    ...
    if (invalidClientIds.rows[0].count > 0) {
...
async function testOAuthConnection() {
  ...
  try {
    ...
    console.log(isConnected 
      ? chalk.green('  ✓ OAuth tokens are present')
      : chalk.yellow('  ⚠ OAuth tokens not found or expired'));
    
    if (isConnected) {
      // Test actual connection
      const testResult = await simpleOAuth.testConnection();
      console.log(testResult
        ? chalk.green('  ✓ OAuth connection test successful')
        : chalk.yellow('  ⚠ OAuth connection test failed - tokens may be expired'));
      
      if (testResult) {
        // Try to get calendars
        try {
          const calendars = await simpleOAuth.getCalendars();
          console.log(chalk.green(`  ✓ Successfully retrieved ${calendars.length} calendars`));
          
          // List first few calendars
          calendars.slice(0, 3).forEach(cal => {
            console.log(chalk.gray(`    • ${cal.summary} (${cal.id})`));
          });
        } catch (calError: any) {
          console.log(chalk.yellow(`  ⚠ Could not retrieve calendars: ${calError.message}`));
        }
      }
    ...
    console.log(chalk.cyan('\n  ℹ To authenticate with Google, visit:'));
...
async function fixCommonIssues(issues: string[]) {
...
// Helper function to check and refresh OAuth tokens
async function refreshOAuthTokens() {
  console.log(chalk.yellow.bold('\n7. Checking OAuth Token Refresh...'));
  
  try {
    if (simpleOAuth.isConnected()) {
      await simpleOAuth.refreshTokensIfNeeded();
      console.log(chalk.green('  ✓ OAuth tokens refreshed successfully'));
      return true;
    } else {
      console.log(chalk.yellow('  ⚠ No tokens to refresh - authentication required'));
      return false;
    }
  } catch (error: any) {
    console.log(chalk.red(`  ✗ Token refresh failed: ${error.message}`));
    return false;
  }
}
  ...
  try {
    ...
    if (userCount.rows[0].count === '0') {
      ...
      const result = await pool.query(`
        ...
        RETURNING id
      `, ['admin', '$2b$10$hash', 'Default Therapist', 'therapist', 'admin@therapy.local']);
      
      if (result.rows.length > 0) {
        console.log(chalk.green('    ✓ Created default therapist user'));
      ...
      console.log(chalk.gray('  • Users already exist in database'));
    }
  } catch (error: any) {
    console.log(chalk.red(`  ✗ Failed to create sample data: ${error.message}`));
  }
}
...
async function runDiagnostics() {
  try {
    ...
    console.log(oauthConnected
      ? chalk.green('✓ OAuth connection active')
    ...
    if (!oauthConnected) {
      ...
      console.log(chalk.gray('   Visit the OAuth URL shown above'));
    }
