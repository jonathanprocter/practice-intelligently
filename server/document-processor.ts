...
export class DocumentProcessor {
  ...
  async processDocument(filePath: string, originalName: string): Promise<ProcessedDocument> {
    ...
    try {
      ...
      // Generate progress note if we have enough information
      let progressNote: ProgressNote | undefined;
      let sessionNoteId: string | undefined;
      let appointmentId: string | undefined;
      let clientId: string | undefined;
      
      if (detectedInfo.clientName || detectedInfo.sessionDate) {
        try {
          // Generate structured progress note
          progressNote = await this.generateProgressNote(
            extractedText,
            detectedInfo.clientName || 'Unknown Client',
            detectedInfo.sessionDate || new Date().toISOString()
          );
          
          // Auto-create session note in database if we have a valid client
          const autoCreateResult = await this.autoCreateSessionNote(
            progressNote,
            detectedInfo.clientName,
            detectedInfo.sessionDate
          );
          
          if (autoCreateResult) {
            sessionNoteId = autoCreateResult.sessionNoteId;
            appointmentId = autoCreateResult.appointmentId;
            clientId = autoCreateResult.clientId;
            console.log(`✅ Auto-created session note: ${sessionNoteId}`);
          }
        } catch (error) {
          console.error('Failed to generate or save progress note:', error);
        }
      }
...
Session Date: ${sessionDate}`;
  ...
  async autoCreateSessionNote(
    progressNote: ProgressNote,
    clientName?: string,
    sessionDate?: string
  ): Promise<{ sessionNoteId: string; appointmentId?: string; clientId?: string } | null> {
    try {
      if (!clientName) {
        console.error('Cannot auto-create session note without client name');
        return null;
      }

      // Find or create client
      let clientId = await storage.getClientIdByName(clientName);
      ...
      if (!clientId) {
        ...
        // Get first therapist ID (admin user)
        const therapists = await storage.getUser('e66b8b8e-e7a2-40b9-ae74-00c93ffe503c');
        const therapistId = therapists?.id || 'e66b8b8e-e7a2-40b9-ae74-00c93ffe503c';
      ...
      // Create comprehensive session note content
      const sessionContent = `# Comprehensive Clinical Progress Note
...
` +
        `## Tonal Analysis
${progressNote.tonalAnalysis}

` +
        `## Key Points
${progressNote.keyPoints.join('\n- ')}

` +
        `## Significant Quotes
${progressNote.significantQuotes.join('\n- ')}

` +
        `## Narrative Summary
${progressNote.narrativeSummary}

` +
        `---
*AI-generated tags:* ${progressNote.aiTags?.join(', ') || 'N/A'}`;
      ...
      if (sessionDate && clientId) {
        ...
        const matchingAppointment = appointments.find(apt => {
          ...
          return (
            aptDate.getFullYear() === sessionDateObj.getFullYear() &&
            aptDate.getMonth() === sessionDateObj.getMonth() &&
            aptDate.getDate() === sessionDateObj.getDate()
          );
        });

        if (matchingAppointment) {
          appointmentId = matchingAppointment.id;
          // Update session note with appointment link
          await storage.updateSessionNote(sessionNote.id, {
            eventId: matchingAppointment.googleEventId || undefined
          });
          console.log(`Linked session note to appointment: ${appointmentId}`);
        } else {
      ...
      console.error('Error auto-creating session note:', error);
      return null;
    }
  }
}
