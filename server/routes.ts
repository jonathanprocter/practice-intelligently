    ...
    try {
      ...
      for (const session of sessions) {
        try {
          ...
          createdCount++;
          console.log(`✅ Created session note ${sessionNote.id} for session ${session.sessionId}`);

        } catch (sessionError: any) {
          console.error(`Error creating session note for ${session.sessionId}:`, sessionError);
          processedSessions.push({
            sessionId: session.sessionId,
            title: session.title,
            date: session.date,
      ...
      res.json({
        success: true,
        message: `Successfully processed ${createdCount} of ${sessions.length} sessions`,
        processedSessions: processedSessions,
        totalCreated: createdCount,
        totalFailed: sessions.length - createdCount
      });
